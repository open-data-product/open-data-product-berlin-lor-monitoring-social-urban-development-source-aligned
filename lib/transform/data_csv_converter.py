...
def convert_data_to_csv(source_path, results_path, clean=False, quiet=False):
    ...
    for subdir, dirs, files in sorted(os.walk(source_path)):
        ...
        for file_name in sorted(files):
            ...
            if (
                re.search(r"\d{4}-\d{2}-1.xlsx", file_name) is not None
                ...
                re.search(r"\d{4}-\d{2}-2-2.xlsx", file_name) is not None
                or re.search(r"\d{4}-\d{2}-4-2.xlsx", file_name) is not None
            ):
                source_file_paths_district_regions.append(source_file_path)
            elif (
                re.search(r"\d{4}-\d{2}-2-3.xlsx", file_name) is not None
                or re.search(r"\d{4}-\d{2}-4-3.xlsx", file_name) is not None
            ):
                source_file_paths_districts.append(source_file_path)

        if (
            len(source_file_paths_planning_areas) > 0
            and len(source_file_paths_district_regions) > 0
            and len(source_file_paths_districts) > 0
        ):
            convert_file_to_csv_planning_areas(
                source_file_paths_planning_areas, clean=clean, quiet=quiet
            )
            convert_file_to_csv_district_regions(
                source_file_paths_district_regions, clean=clean, quiet=quiet
            )
            convert_file_to_csv_districts(
                source_file_paths_districts, clean=clean, quiet=quiet
            )
...
def convert_file_to_csv_planning_areas(source_file_paths, clean=False, quiet=False):
    ...
    file_path_csv = os.path.join(
        os.path.dirname(source_file_paths[0]),
    ...
    if clean or not os.path.exists(file_path_csv):
        ...
        try:
            for source_file_path in source_file_paths:
                source_file_name, source_file_extension = os.path.splitext(
                    source_file_path
                )
                ...
                if re.search(r"\d{4}-\d{2}-1", source_file_name) is not None:
                    ...
                    if year == "2019" or year == "2021":
                        drop_columns = [
                            "name",
                            "inhabitants",
                            "average_and_standard_deviation",
                            "s2_percentage_long_term_unemployed",
                            "d2_percentage_long_term_unemployed",
                        ]
                    else:
            ...
            dataframe = (
                pd.concat([df.set_index("id") for df in dataframes], axis=1)
                .reset_index()
                .dropna()
            )
